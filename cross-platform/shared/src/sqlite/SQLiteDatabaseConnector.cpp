...
bool SQLiteDatabaseConnector::insertUser(User user)
{
    ...
    sqlite3_bind_text(stmt, 2, user.getLogin().c_str(), user.getLogin().length(), SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, user.getName().c_str(), user.getName().length(), SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 4, user.getAvatar().c_str(), user.getAvatar().length(), SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 5, user.getLocation().c_str(), user.getLocation().length(), SQLITE_TRANSIENT);
...
bool SQLiteDatabaseConnector::insertRepos(std::vector<Repo> repos)
{
    ...
    for (std::vector<Repo>::iterator it = repos.begin(); it != repos.end(); ++it)
    {
        ...
        sqlite3_bind_text(stmt, 2, it->getName().c_str(), it->getName().length(), SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 3, it->getDescription().c_str(), it->getDescription().length(), SQLITE_TRANSIENT);
        sqlite3_bind_int(stmt, 4, it->isPrivateRepo() ? 1 : 0);
        sqlite3_bind_int(stmt, 5, it->isFork() ? 1 : 0);
        sqlite3_bind_text(stmt, 6, it->getLanguage().c_str(), it->getLanguage().length(), SQLITE_TRANSIENT);
